...
-- AND course_id  = ( SELECT  c.id FROM "COURSE" c 
-- 				           LEFT JOIN "USER" u ON c.user_id = u.id
-- 				           WHERE u.name = '李燕容');
...
-- INNER JOIN "COACH" c ON cls.coach_id = c.user_id
...
-- SELECT s.name AS 專長名稱, count(cls.coach_id) AS coach_total
-- FROM "COACH_LINK_SKILL" cls 
-- INNER JOIN	"SKILL" s ON cls.skill_id = s.id
-- GROUP BY s.name
-- ORDER BY count(cls.coach_id)
-- limit 1;
...
-- FROM "CREDIT_PURCHASE" cp 
-- WHERE purchase_at BETWEEN '2024-11-01' AND '2024-12-01';
